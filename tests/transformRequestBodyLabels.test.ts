...
describe('transformRequestBody block labels (for console-output reporting)', () => {
  ...
  it('keeps Read file paths exact for local tool execution', async () => {
    const originalPath = 'examples/fixtures/held-out/fintech/merchant-payout-schedule.ts';
    const body = {
      messages: [
        {
          role: 'assistant',
          content: [{ type: 'tool_use', id: 't1', name: 'Read', input: { file_path: originalPath } }],
        },
      ],
    };

    await transformRequestBody(body, new RenameMap(100, 60_000, 'stealth', 0));

    expect(body.messages[0].content[0].input.file_path).toBe(originalPath);
  });
