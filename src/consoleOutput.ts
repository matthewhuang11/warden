...
export function printBanner(port: number, upstreamBaseUrl: string, obfuscationEnabled: boolean): void {
  if (!obfuscationEnabled) {
    console.log(`⚠ Warden is running on :${port} — obfuscation is DISABLED, traffic is passing straight through to ${upstreamBaseUrl}`);
    return;
  }
...
export function printUpstreamUnreachableWarning(upstreamBaseUrl: string, error: string): void {
  console.error(
    [
      '',
      `⚠ Warning: could not reach upstream ${upstreamBaseUrl} (${error}).`,
      '',
